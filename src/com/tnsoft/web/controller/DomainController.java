...
public class DomainController extends BaseController {
	...
	public String domain(Model model) {
		Utils.saveLog(lg.getUserId(), "查看站点的列表", lg.getDomainId());
...
//			try {
//				domain = (NDADomain) session.get(NDADomain.class, Integer.parseInt(id));
...
//			}
			...
			if (lg.getDomain().getDomainPath().equals("/")) {
				...
				items.add(item);
				List<NDADomain> domains = domainService.getAll();
				SelectItem item2;
				for (NDADomain nd : domains) {
	...
	public String saveDomain(Model model, String name, String id, String phone, String address, String description,
		...
		try {
			...
			Date now = new Date();
			if (!StringUtils.isEmpty(id) && Integer.parseInt(id) > 0) {
				NDADomain domain = domainService.getById(id);
				if (domain != null) {
					domain.setName(name);
				...
				// 报警设置
				NDAAlertLevel l = new NDAAlertLevel();
				...
				NDAAlertLevel l2 = new NDAAlertLevel();
	...
	public String deleteDomain(Model model, String id, int mode, RedirectAttributes attr) {
		...
		try {
			...
			if (!StringUtils.isEmpty(id)) {
				NDADomain domain = (NDADomain) db.get(NDADomain.class, Integer.parseInt(id));
				if (domain != null) {
					if (mode == 1) {
						domain.setLastModified(now);
						attr.addFlashAttribute("message", "站点停用成功");
						Utils.saveLog(lg.getUserId(), "停用站点", lg.getDomainId());

						domain.setStatus(Constants.DomainState.STATE_DISABLE);
					} else {
						domain.setLastModified(now);
						attr.addFlashAttribute("message", "站点恢复成功");
						Utils.saveLog(lg.getUserId(), "恢复站点", lg.getDomainId());
	...
	private List<NDADomain> query(DbSession session, String where, String order, int offset, int limit) {
		...
		List<NDADomain> list = domainService.getByHQLWithLimits(offset, limit, sql);
		
		if (!list.isEmpty()) {
			for (NDADomain temp : list) {
		...
		return Collections.<NDADomain>emptyList();
	}
